...
function [x, theta, testdata] = dmrg2(A, B, tol, varargin)
    ...
    for i = d:-1:2
        ...
        phixax(i) = rightreduce_matrix(phixax(i+1), crx{i}, crA(i), ...
                                       crx{i}, rx(i), n(i), rx(i+1), 1, ...
                                       ra(i), ra(i+1), rx(i), n(i), ...
                                       rx(i+1));

        phixbx(i) = rightreduce_matrix(phixbx(i+1), crx{i}, crB(i), ...
                                       crx{i}, rx(i), n(i), rx(i+1), 1, ...
                                       rb(i), rb(i+1), rx(i), n(i), ...
                                       rx(i+1)); %%%%%
    ...
    theta = rightreduce_matrix(phixax(2), crx{1}, crA(1), crx{1}, ...
                               rx(1), n(1), rx(1+1), 1, ra(1), ra(1+1), ...
                               rx(1), n(1), rx(1+1));
    theta = squeeze(theta{1});
    tmp = rightreduce_matrix(phixbx(2), crx{1}, crB(1), crx{1}, ...
                             rx(1), n(1), rx(1+1), 1, rb(1), rb(1+1), ...
                             rx(1), n(1), rx(1+1));
    tmp = squeeze(tmp{1});
    theta = theta/tmp;
    ...
    while swp <= nswp || dir > 0
        ...
        res_prev = norm(local_matvec(sol_prev, rx1, nloc, rx2, b, rx1, ...
                                     nloc, rx2, Phia1, A1, Phia2, 1, ...
                                     ra1, ra2)- ...
                        local_matvec(sol_prev, rx1, nloc, rx2, b, rx1, ...
                                     nloc, rx2, Phib1, B1, Phib2, 1, ...
                                     rb1, rb2)*diag(theta)); %%%%%
        ...
        if rx1*nloc*rx2 < max_full_size % Full solution
            ...
            if usemex
                ...
                [sol, theta, num_matvecs] = ...
                    eig3d_primme(Phi1m, A1m, Phi2m, real_tol, b, ...
                                 sol_prev, local_iters);
            else
                ...
                [sol, theta] = eigs(Axx, Bxx, b, 'smallestabs', ...
                                    'Tolerance', real_tol, ...
                                    'MaxIterations', max_full_size, ...
                                    'StartVector', sol_prev, ...
                                    'FailureTreatment', 'keep', ...
                                    'IsSymmetricDefinite', 1); %%%%%
            end
        end
        ...
        if ~strcmp(trunc_norm, 'fro')
            ...
            res_new = norm(local_matvec(sol_prev, rx1, nloc, rx2, b, ...
                                        rx1, nloc, rx2, Phia1, A1, ...
                                        Phia2, 1, ra1, ra2)- ...
                           local_matvec(sol_prev, rx1, nloc, rx2, b, ...
                                        rx1, nloc, rx2, Phib1, B1, ...
                                        Phib2, 1, rb1, rb2) ...
                           *diag(theta)); %%%%%
        end
        ...
        if (dir > 0 && i > d-numblocks+1) || (dir < 0 && i < 1)
            ...
            if verb > 0
                fprintf(['=dmrg_eig= sweep %d, max_dx: %3.3e, ', ...
                         ...
                         'mv: %d\n'], swp, max_dx, max_res, ...
                        sqrt(rx(1:d)'*(n.*rx(2:d+1))/sum(n)), ...
                        sum(theta), max_matvecs);
            end
